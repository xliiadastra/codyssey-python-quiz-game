...
캡처합니다. 현재 버전에서는 목록 아래 삭제 여부가 표시되므로 삭제하지
않으려면 `n`을 입력합니다.
...
메뉴에서 `1`을 선택하고 풀 문제 수를 입력합니다. 현재 버전은 문제 순서가
무작위이므로 화면에 표시된 문제를 보고 정답을 선택합니다. 정답 입력 대신
`H`를 입력하면 힌트가 나오며 문제당 5점이 차감됩니다. 마지막 결과와 최종
점수가 보이는 화면을 캡처합니다.
...
현재 버전에서는 날짜·문제 수·힌트 수·점수가 포함된 전체 기록도 함께 표시됩니다.
