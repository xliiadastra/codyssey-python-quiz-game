...
예를 들어 같은 `Quiz` 클래스로 문제 내용과 힌트가 서로 다른 객체 여러 개를
만듭니다.
...
9. `random.sample()`은 원본 목록을 바꾸지 않고 원하는 수만큼 중복 없이
   무작위 문제를 고릅니다.
10. 힌트는 문제당 한 번만 감점하고, 기본 점수에서 힌트 수 × 5점을 빼되
    `max(0, ...)`으로 최종 점수가 음수가 되지 않게 합니다.
11. 각 게임 결과는 날짜·시간과 함께 딕셔너리로 만들어 `score_history`
    리스트에 추가하므로 최고 점수와 별개로 모든 기록이 남습니다.
12. 퀴즈 삭제는 화면 번호에서 1을 빼 리스트 인덱스로 바꾸고, 두 번 확인한
    뒤 `del`로 제거하여 즉시 JSON에 저장합니다.
13. 서로 다른 문제 수로 얻은 최종 점수가 같으면 `(점수, 정답 수, 문제 수)`를
    비교해 더 많은 문제를 맞힌 기록을 최고 기록으로 남깁니다.
...
> 커밋하고 풀이 기능을 별도 브랜치에서 만든 뒤 main에 병합했습니다. 보너스로
> 문제 수 선택과 랜덤 출제, 힌트 감점, 퀴즈 삭제, 전체 점수 기록도 구현했습니다.
...
9. 문제 수를 선택하고 `H`를 입력해 5점이 차감되는지 확인합니다.
10. 목록에서 퀴즈 하나를 삭제한 뒤 재실행해 삭제 상태가 유지되는지 확인합니다.
11. 두 번 이상 플레이하고 점수 확인에서 모든 기록이 남는지 확인합니다.
...
- `random.sample()`을 사용한 이유와 `random.shuffle()`과의 차이는?
- 힌트를 여러 번 요청해도 한 번만 감점되는 이유는?
- 최고 점수와 전체 게임 기록을 별도로 저장하는 이유는?

이 질문들에 코드 없이 말로 답할 수 있으면 과제의 핵심과 보너스 구현을
이해한 것입니다.
